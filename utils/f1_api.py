...
def get_driver_standings(limit=20):
    """
        ...
        return [
            {
                ...
                'points':         int(float(d['points'])),
                'wins':           int(d['wins']),
                'constructor_id': d['Constructors'][0]['constructorId'],
            }
            for d in raw
        ]
    except Exception as e:
        print(f"F1 API Error (get_driver_standings parse): {e}")
        return []
...
def get_constructor_standings(limit=10):
    """
    ...
    try:
        ...
        return [
            {
                'pos':            int(c['position']),
                'constructor_id': c['Constructor']['constructorId'],
                'name':           c['Constructor']['name'],
                'short_name':     _CONSTRUCTOR_NAMES.get(
                                      c['Constructor']['constructorId'],
                                      c['Constructor']['name'][:8].upper()
                                  ),
                'points':         int(float(c['points'])),
                'wins':           int(c['wins']),
            }
            for c in raw
        ]
